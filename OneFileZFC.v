...
(* here we prove theorem for \in only *) (* then add and = *)
Require Import Bool.
...
(*
| fsunion :Terms -> Terms
| fspair : Terms -> Terms -> Terms
*)
.

Inductive AtomFm : Type :=
| IN : Terms -> Terms -> AtomFm
(*
| EQ : Terms -> Terms -> AtomFm
*)
.
...
Inductive Fm :=
 ...
 |Disj:Fm->Fm->Fm
 |Impl:Fm->Fm->Fm
 |Fora(x:SetVars)(f:Fm): Fm
 |Exis(x:SetVars)(f:Fm): Fm
.
...
Definition In (x1 x2:X) := 
match x1, x2 with
| Se n1, Se n2 => n1 < n2
end.

Definition SetVars_eqb r xi :=
match r, xi with
| Va r, Va xi => Nat.eqb r xi
end.

Definition cng (val:SetVars -> X) (xi:SetVars) (m:X) :=
 (fun r:SetVars =>
 match SetVars_eqb r xi with
 | true => m
 | false => (val r)
 end).
...
Fixpoint foI (val : SetVars -> X) (f : Fm) {struct f} : Omega :=
   match f with
   | Atom (IN x y)=> In (teI val x) (teI val y)
   | Bot => OFalse
...
Definition Neg (A:Fm):Fm := Impl A Bot.
Definition Top:Fm := Neg Bot.

(* Module PredFormulasNotationsASCII. *)
 Declare Scope pretxtnot.
...
:=
   match u with
   | FVC s => if (SetVars_eqb s xi) then t else FVC s
   (* | FSC f t0 => FSC f (Vector.map (substT t xi) t0) *)
   end.


Fixpoint isParamT (xi : SetVars) (t : Terms) : bool :=
   match t with
   | FVC s => SetVars_eqb s xi
(*   | FSC f t0 => Vector.fold_left orb false (Vector.map (isParamT xi) t0) *)
   end.

Fixpoint isParamF (xi : SetVars) (f : Fm) : bool :=
   match f with
   | Atom (IN x y) => isParamT xi x || isParamT xi y
(* Vector.fold_left orb false (Vector.map (isParamT xi) t0) *)
   | Bot => false
...
(* Substitution *)
Fixpoint substF (t : Terms) (xi : SetVars) (u : Fm)
 {struct u} : option Fm
 :=let f := (substF t xi) in
   let g:= (substT t xi) in
   match u with
   | Atom (IN x y) => Some (Atom (IN (g x) (g y)) )
(* Some (Atom p (map (substT t xi) t0))  *)
   | Bot => Some Bot
   | Conj u1 u2 =>
...
Inductive PRECA : Fm -> Type :=
| PRO  :> forall A, (PROCA A) -> (PRECA A)
| Ha12 : forall (ph: Fm) (t:Terms) (xi:SetVars)
 (r:Fm) (s:(substF t xi ph)=Some r), PRECA ((Fora xi ph) --> r)
| Ha13 : forall (ph: Fm) (t:Terms) (xi:SetVars)
 (r:Fm) (s:(substF t xi ph)=Some r), PRECA (r --> (Exis xi ph) )
| Hb1  : forall (ps ph: Fm) (xi:SetVars) (H:isParamF xi ps = false),
PRECA (Impl (Fora xi (Impl ps ph)) (Impl ps (Fora xi ph)) )
| Hb2  : forall (ps ph: Fm) (xi:SetVars) (H:isParamF xi ps = false),
...
Definition NotParamC xi ctx :=
 forall F : Fm, InL F ctx -> isParamF xi F = false.

Section PREPR.
Context (ctx:list Fm).
Inductive PREPR : Fm -> Type :=
| hyp_E (A : Fm) : (InL A ctx) -> (PREPR A)
| Hax_E :> forall (A : Fm), (PRECA A) -> (PREPR A)
| MP_E (A B: Fm) : (PREPR A) -> (PREPR (Impl A B)) -> (PREPR B)
| GEN_E (A: Fm) (xi :SetVars) (nic:NotParamC xi ctx)
